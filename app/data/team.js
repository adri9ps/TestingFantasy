...
class Team {
   ...
   static getRandomTeamTactic(){
    ...
    static createRandomTeam(listPossiblePlayers,tactic,teamName,teamValue){
        ...
        //BUG 7 - falta añadir metodo
        let jugadoresValidos = listPossiblePlayers.filter(player => player.value < teamValue);
        if (jugadoresValidos.length < 11) {
            throw Error('Faltan jugadores en el equipo');
        }
        ...
        //BUG 5 //
       let valorEquipo = teamValue + 1;
       let newTeam = null;
        while(valorEquipo > teamValue ){
