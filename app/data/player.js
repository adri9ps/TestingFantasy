...
class Player{
    ...
    static fromDatasetJSONObject(playerId,jsonObject){
        ...
        // SOLUCIÓN BUG 1 -- DE STRING A FLOAT 
        let valor = jsonObject[constants.market_value];
        let numeroFinal;
        let letra = valor.slice(valor.length-1);
        if(letra === 'K'){
            let numero1 = valor.slice(1,-1);
            let numeroFloat1 = parseFloat(numero1);
            numeroFinal = numeroFloat1*1000;
        }else if(letra === 'M'){
            let numero2 = valor.slice(1,-1);
            let numeroFloat2 = parseFloat(numero2);
            numeroFinal = numeroFloat2*1000000;
        }
        player.value = numeroFinal;
